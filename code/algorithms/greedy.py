"""
This file contains the Greedy class which implements a greedy algorithm for finding paths between chips.
This greedy algorithm is based on the Manhattan distance metric.
"""
...
class Greedy:
    """Assigns the best possible value to each node, one by one"""
    ...
    def add_connection(self, start_gate, end_gate):
        ...
        # Look if dy is negative of not to decide which way to go
        if dy > 0:
            j = 1
        elif dy == 0:
            j = 0
        ...
        for _ in range(abs(dx)):
            ...
            # Make sure z-coordinate goes down again if possible
        ...
        for _ in range(abs(dy)):
            ...
            # Make sure the z-coordinate goes down again if possible
            while z != 0 and self.chip.grid[x][y][z_new] == 0:
                z = z_new
                path.append((x, y, z))
                self.chip.grid[x][y][z] += 1
